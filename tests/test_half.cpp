...
namespace {
    ...
    void test_one_point_oh() {
        const std::uint16_t result = cooker::to_half(1.0F);
        // 1.0f in IEEE 754 half is 0x3C00.
        check(result == 0x3C00U, "1.0");
    }

    void test_zero_point_five() {
        const std::uint16_t result = cooker::to_half(0.5F);
        // 0.5f in IEEE 754 half is 0x3800.
        check(result == 0x3800U, "0.5");
    }

    void test_zero() {
        const std::uint16_t result = cooker::to_half(0.0F);
    ...
    void test_largest_finite() {
        ...
        const std::uint16_t result = cooker::to_half(65504.0F);
    ...
    void test_clamps_past_largest() {
        ...
        const std::uint16_t result = cooker::to_half(66000.0F);
    ...
    void test_infinity_clamps() {
        const std::uint16_t result =
            cooker::to_half(std::numeric_limits<float>::infinity());
        check(result == 0x7BFFU, "an infinity clamps to the largest finite half");
    }

    void test_nan_clamps() {
        const std::uint16_t result =
            cooker::to_half(std::numeric_limits<float>::quiet_NaN());
    ...
    void test_denormal() {
        ...
        const std::uint16_t result = cooker::to_half(denormal);
    ...
    void test_tiny_value_flushes() {
        ...
        const std::uint16_t result = cooker::to_half(tiny);
    ...
    void test_mantissa_truncation() {
        ...
        const std::uint16_t result = cooker::to_half(1.0F + (1.0F / 1024.0F));
