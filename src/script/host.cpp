...
namespace engine::script {
    ...
    /**
     * @cond
     * Declared before EntityHandle uses them. Doxygen skips implementation.
     */
    struct EntityHandle;
    /// @endcond
     ...
     * the script runs on. It holds no state of its own: the world and the
     * registry come from the host, and the entity number is the whole value.
     */
    struct EntityHandle {
    ...
    namespace {
        ...
        [[nodiscard]] sol::object read_component(const EntityHandle& self,
            ...
            const scene::ComponentOps* ops = self.components->find(component);
            if (ops == nullptr || ops->const_instance == nullptr) {
                return sol::lua_nil;
            }
            const void* instance = ops->const_instance(self.world->registry(), self.id);
            if (instance == nullptr) {
                return sol::lua_nil;
            }
        ...
        [[nodiscard]] bool write_component(const EntityHandle& self,
            ...
            const scene::ComponentOps* ops = self.components->find(component);
            if (ops == nullptr || ops->instance == nullptr) {
                return false;
            }
            void* instance = ops->instance(self.world->registry(), self.id);
            if (instance == nullptr) {
                return false;
            }
    ...
    void Host::bind_entity() {
        impl_->lua.new_usertype<EntityHandle>(
            ...
            [](const EntityHandle& self, const std::string& component) {
                const scene::ComponentOps* ops = self.components->find(component);
                return ops != nullptr && ops->has(self.world->registry(), self.id);
            },
    ...
    void Host::update(scene::World& world, double seconds) {
        ...
        for (const auto [entity, component] : registry.view<const ScriptComponent>().each()) {
            ...
            instance.env["entity"] =
                EntityHandle{ entity, &world, impl_->components };
